...
gboolean parsed_fields_ok(const ParsedFieldsF *pf) { return pf->ok; }
...
struct ParsedAirport {
  gchar *fields[8]; // 0: code, 1: name, 2: city, 3: country, 4: latitude, 5:
                    // longitude, 6: icao, 7: type
  gboolean ok;
  gchar *line;
};
...
gboolean parsed_airport_ok(const ParsedAirportF *pf) { return pf->ok; }
...
const gchar *parsed_airport_line(const ParsedAirportF *pf) { return pf->line; }

struct ParsedAircraft {
  gchar *fields[6]; // 0: id, 1: manufacturer, 2: model, 3: year, 4: capacity,
                    // 5: range
  gboolean ok;
  gchar *line;
};
...
gboolean parsed_aircraft_ok(const ParsedAircraftF *pf) { return pf->ok; }
...
gboolean parsed_passenger_ok(const ParsedPassengerF *pf) { return pf->ok; }
...
struct ParsedReservation {
  gchar *fields[8]; // 0: reservation id, 1: flight ids, 2: document number, 3:
                    // seat, 4: price, 5: extra luggage, 6: priority boarding,
                    // 7: qr code
  gboolean ok;
  gchar *line;
};

// Safe version of parseFlightIds()
// --------------------------------
// Converts a string like "['ABC123', 'DEF456']"
// into a NULL-terminated gchar** array: ["ABC123", "DEF456", NULL].
//
// Returns NULL if the input field is NULL or empty.
//
gchar **parseFlightIds(const gchar *field) {
  ...
  for (int i = 0; raw_ids[i]; i++) {
    ...
    /* invalid flight id → invalidate whole list */
    if (*t == '\0') {
      g_free(t);
...
ParsedReservationF *parseReservationLineRaw(const gchar *line) {
    ParsedReservationF *pr = g_malloc0(sizeof(ParsedReservationF));
    pr->ok = FALSE;

    if (!line || *line == '\0') return pr;
...
ParsedReservationF *parseReservationLineRaw(const gchar *line) {
  ...
  // If quotes were not closed → invalid
  if (in_quotes || idx >= 8) {
    g_string_free(field, TRUE);
    parsed_reservation_free(pr);
    return NULL;
...
gboolean parsed_reservation_ok(const ParsedReservationF *pr) { return pr->ok; }
